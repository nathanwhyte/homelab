...
- VLM: **cloud primary** via `http://chat-ollama.llama.svc:11434/v1` (llama-ns cloud routing, Bearer from `ollama-api-key`) — currently `gemma4:31b-cloud` (resync-era swap 2026-07-07: `reasoning_effort` is an unknown config field in OV v0.4.7 and crashloops the server, so a non-thinking model is required; steady-state target `qwen3.5:cloud`, vision, 256K ctx — revert after the compendium resync). `vlm.backup` = local `http://llamacpp-vlm.viking.svc/v1` (selector `vlm-pool: "true"` → `llamacpp-cuda-ov`), automatic failover on 429/5xx/timeout (IDEA-1050, 2026-07-05)

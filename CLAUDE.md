...
| wemby | agent                           | GTX 1060 6 GB    | **none.** Prior: `reranker-bge` (bge-reranker-v2-m3 cross-encoder for BUG-1016) deployed 2026-08-26, scaled to 0 on 2026-08-27, **retired 2026-09-18** — never wired into `ov.conf`, never served a request. Measured worse at the OV level: 38.2% → 20.6% top-1 at calibrated thresholds (−9.0 and −12.0, after the raw-logit-vs-`threshold: 0.2` mismatch was corrected) and 20–36 s/query, which disqualifies it for interactive search on its own. It also cannot address BUG-1016's dominant failure mode — exact-ID queries, which no reranker can promote because retrieval never returns them. `embedder-qwen-cuda` relocated to manu 2026-07-17 (IMPR-1077); `embedder-llamacpp` deleted 2026-07-04. The card also has an unresolved hard power-off at 84-85 °C under sustained load |
...
- v0.4.20 posture (IMPR-1152, live since 2026-09-15): the v0.4.10 settings above carry over unchanged. Relevant changes since our previous v0.4.10 deployment — tenant-scoped trusted-mode requests now have their `account_id`/`user_id` validated rather than merely honoured; a `user` memories path with no user-id segment (since v0.4.17) and bare-path URIs with no `viking://` scheme (since v0.4.16) are rejected as `INVALID_URI`; `ov set-tags` is a silent no-op (compendium BUG-1140); and `ov mv` onto an existing target was measured returning 500 and deleting the target (BUG-1141) — treat `ov cp` with the same caution, though only `mv` was measured
...
- **Markdown filenames**
  ...
  - Uppercase/title-case is reserved for: repo-root entry points (`README.md`, `HARDWARE.md`, `GPU_AND_AI_REVIEW.md`, `LICENSE.md`), agent-instruction files matched by basename at any depth (`CLAUDE.md`, `AGENTS.md`, `SKILL.md`), `README.md` anywhere, and the lifecycle markers `TORN-DOWN.md` / `RETIRED.md`.

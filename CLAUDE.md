...
| OV LLM (NVIDIA)                 | viking    | `llamacpp-cuda-llm.viking.svc`    | 80→8000                      | manu                | `llamacpp-cuda-ov`; Qwen3-8B IQ4_XS, ctx=32768, 2 slots; sole VLM backend.                                                                                                                                                    |
...
| Hermes Jump                     | hermes    | `hermes-jump.hermes.svc`          | 22                           | —                   | SSH terminal for Hermes; ephemeral (no persistent PVC).                                                                                                                                                                       |
...
| -------------------- | ------------------- | ----------------------------------------------------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| llamacpp-cuda-ov     | manu / GTX 1080     | Qwen3-8B IQ4_XS, ctx=32768                            | 2     | **Primary VLM** (NVIDIA); replicas=1 always on (1080 VLM-exclusive). ~29-30 tok/s gen, ~414 tok/s prompt eval.                                                                                                                          | `viking/manifests/cuda-llamacpp-deployment.yaml`      |
...
| `viking.nathanwhyte.dev`                  | `openviking:1933`                               | OV `root_api_key`               | OV console; Ingress (see `viking/manifests/ov-console-ingress.yaml`)                                                                                 |
...
- Ports: API 8642 (Bearer auth), dashboard 9119 (session-token, tunnel at `hermes.nathanwhyte.dev`); terminal SSH `hermes-jump.hermes.svc:22` (ephemeral)
...
- Tuning: `server.workers=2`, `embedding.batch_size=128`, `vlm.max_concurrent=2`, `embedding.max_concurrent=4`

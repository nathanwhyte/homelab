...
| wemby | agent                           | GTX 1060 6 GB    | **none.** Prior: `reranker-bge` (bge-reranker-v2-m3 cross-encoder for BUG-1016) deployed 2026-08-26, scaled to 0 on 2026-08-27, **retired 2026-09-18** — never wired into `ov.conf`, never served a request. It could not address BUG-1016's dominant failure mode (exact-ID queries, unreachable by any reranker), and the "-17.6pp" figure often cited against it is confounded: OV compared raw logits (-8 to -11) against `rerank.threshold: 0.2`, so the ranking was meaningless. The reranker hypothesis is untested, not disproven. `embedder-qwen-cuda` relocated to manu 2026-07-17 (IMPR-1077); `embedder-llamacpp` deleted 2026-07-04. The card also has an unresolved hard power-off at 84-85 °C under sustained load |

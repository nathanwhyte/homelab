...
- v0.4.20 posture (IMPR-1152, live since 2026-09-15): the v0.4.10 settings above carry over unchanged. New with this version — trusted-mode requests have `account_id`/`user_id` validated on every call, uid-less `user/memories/`-style URIs and bare-path URIs are rejected, `ov set-tags` is a silent no-op (compendium BUG-1140) and `ov mv`/`cp` onto an existing target can destroy it (BUG-1141)

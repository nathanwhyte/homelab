...
Patched for the homelab self-hosted Mem0 OSS server. The only change from the
upstream Platform-only plugin is:

1. `base_url` is loaded from env (`MEM0_BASE_URL` / `MEM0_HOST`) or `mem0.json`.
2. `MemoryClient` is constructed with `host=self._config.get("base_url")`,
   allowing Hermes to point at the local mem0-adapter sidecar.

All other behavior is preserved so the patch stays minimal and easy to audit.
"""
...
def _load_config() -> dict:
    ...
    config = {
        ...
        "base_url": os.environ.get("MEM0_BASE_URL") or os.environ.get("MEM0_HOST", ""),
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        return [
            ...
            {
                "key": "base_url",
                "description": "Mem0 API base URL (default: https://api.mem0.ai)",
    ...
    def _get_client(self):
        ...
        with self._client_lock:
            ...
            try:
                ...
                base_url = self._config.get("base_url") if self._config else ""
                self._client = MemoryClient(
                    api_key=self._api_key,
                    host=base_url or "https://api.mem0.ai",
                )
                return self._client
            except ImportError:
                raise RuntimeError(

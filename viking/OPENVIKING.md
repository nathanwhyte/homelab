...
Even with `--wait`, the semantic queue processor runs tasks concurrently based on `embedding.max_concurrent` and `vlm.max_concurrent`. When multiple tasks target the same AGFS subtree (e.g., bulk sync of `bugs/mage/`), AGFS EXACT/TREE path locks (logged as POINT/SUBTREE) contend and time out. Note: this is the AGFS transaction layer, not the VectorDB — distinct from the embedded LevelDB VectorDB process lock. Current production concurrency: `vlm.max_concurrent=4` (matches VLM server's `--parallel 4`, bumped from 2 in IDEA-1042 Phase 4 cutover; the IDEA-009 over-commit failure was specifically a client>server mismatch, not `--parallel 4` itself), `embedding.max_concurrent=6` (bumped from 4 in IDEA-1042 Phase 3 to match Qwen embedder `--parallel 4`; over-provisioned but harmless since embedder is no longer the bottleneck). Lock contention was resolved by moving AGFS to S3/Garage (process-local locks, not shared-filesystem locks). The `vlm.max_concurrent=1` guidance from 2026-05-30 predated the S3 cutover and is no longer necessary.
...
**Embedder configuration** (IDEA-1042 cutover 2026-06-22, commit `2621fca`; wemby CUDA 2026-06-29 → **back to timmy ROCm 2026-07-06**): primary is now `embedder-qwen` (ROCm backend, IMPR-1040 backend-neutral rename) on timmy's RX 9070 XT — `--parallel 1 --ctx-size 8192 --batch-size 4096` (8192 tokens/slot, Qwen3-Embedding-4B Q8_0 GGUF, 2560-dim, last-token pooling, ROCm), co-resident with Ollama on the 9070 XT. Migrated off wemby's GTX 1060 after wemby's failing charging cable/port kept hard-dropping the node (repeated `NodeStatusUnknown`); `embedder-qwen-cuda` on wemby (`--batch-size 512`, 1060-VRAM-constrained) is retained at replicas=0 as the CUDA rollback. TASK-1122 validated backend-invariance — Qwen-4B on this 9070 XT reproduced the wemby numbers exactly. Cutover/rollback runbook: `viking/docs/2026-07-06-embedder-migration-wemby-to-timmy.md`. Per-slot ctx must equal `embedding.max_input_tokens` or chunks above the slot size are rejected with "exceeds available context size" — caught the hard way during the original cutover when `--ctx-size 8192 --parallel 6` produced a 1536-token per-slot budget against 8192-token OV chunks (hence `--parallel 1`).
...
- Embedder: Qwen3-Embedding-4B Q8_0 on `embedder-qwen.viking.svc:8080` (timmy, AMD RX 9070 XT, ROCm since 2026-07-06; 2560-dim, last-token pooling, `--parallel 1 --ctx-size 8192 --batch-size 4096`, co-resident with Ollama). Replaced nomic-embed-text-v1.5 in IDEA-1042 cutover (2026-06-22); ran on wemby CUDA 2026-06-29→07-06, migrated back to timmy after wemby's power kept dropping the node. `embedder-qwen-cuda` (wemby, CUDA, replicas=0) is retained as the rollback path; `embedder-llamacpp` (wemby, nomic 768-dim) was deleted from the cluster on 2026-07-04 (manifest kept in repo for history only).

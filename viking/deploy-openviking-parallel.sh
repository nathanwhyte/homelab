...
# Step 5: Build + push coordinator image (if --build flag)
if [[ "${1:-}" == "--build" ]]; then
    echo "--- Step 5: Building coordinator image ---"
    docker build -t "${COORDINATOR_IMAGE}" "${SCRIPT_DIR}/ov-coordinator/"
    docker push "${COORDINATOR_IMAGE}"
else
    echo "--- Step 5: Skipping build (use --build to rebuild) ---"
fi

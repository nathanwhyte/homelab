...
def run_probe(
    ...
    if not result.response.strip():
        ...
        return result
    # A literal think tag in the answer text means the server's thinking
    # separation broke — the one true garbling signal probe checks can miss,
    # since content checkers only look for their expected substrings
    # (qwen3.5:9b leaked '</think>' plus a stray token and still "passed" the
    # weekday probe on 2026-08-14).
    if "<think>" in result.response or "</think>" in result.response:
        result.failure = "think-tag leak in response text"
        return result
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=3,
        help=(
            "Max sampled attempts per probe; one passing attempt passes the "
            "probe. At nonzero temperature a single-sample probe is a coin "
            "flip (qwen3.5:9b answered 17x23 wrong 1-in-3 at temp 0.3, "
            "2026-08-14, aborting a run) — systematic garbling still fails "
            "every attempt."
        ),
    )
    parser.add_argument(
    ...
    for think in think_modes:
        for probe in PROBES:
            ...
            print(
                ...
                f"({res.wall_s:.1f}s, attempt {res.attempts}/{args.attempts})",
                flush=True,
            )

...
def main():
    ...
    required = {"RCLONE_CONFIG_R2_TYPE", "RCLONE_CONFIG_R2_ACCESS_KEY_ID",
    ...
    if any(not key.startswith("RCLONE_CONFIG_R2_") or not isinstance(value, str) or not value
    ...
    command = ["kubectl", "apply", "--server-side", "--field-manager=backup-freshness-deploy", "-f", "-"]
    if args.dry_run:
        command.append("--dry-run=server")
    # Capture both streams: API validation errors must not echo secret payloads.
    result = subprocess.run(command, input=json.dumps(secret), text=True, capture_output=True)
    if result.returncode:
        raise RuntimeError("Secret provisioning failed; output suppressed to protect credentials")
    print("grafana/backup-freshness-r2-credentials: " + ("validated" if args.dry_run else "provisioned"))
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit("Credential provisioning failed; verify the owner-only JSON file and kubectl access")

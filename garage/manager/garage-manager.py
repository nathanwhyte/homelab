...
def download_file(client, bucket_name, remote_key, local_path):
    ...
    try:
        ...
        if error_code == "NoSuchBucket":
            ...
            print(
                f"Error: File '{remote_key}' does not exist in bucket '{bucket_name}'",
                file=sys.stderr,
            )
        else:
...
def list_tree(client):
    ...
    try:
        ...
        error_code = e.response.get("Error", {}).get("Code", "")
        if error_code == "NoSuchBucket":
            print("Error: Bucket does not exist", file=sys.stderr)
        else:
...
Examples:
  ...
  %(prog)s delete mybucket file.txt
    ...
    subparsers.add_parser(
        "list-tree", help="List all buckets and their files in a tree format"
    )
    ...
    download_parser = subparsers.add_parser(
        "download", help="Download a file from a bucket"
    )
    download_parser.add_argument("bucket", help="Bucket name")
    download_parser.add_argument(
        "remote_key", help="Remote object key (path in bucket)"
    )

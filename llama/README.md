...
- **Models**: deepseek-coder-v2:fim (16B-lite MoE Q4, ~13 GB — the single edit-prediction model since the 2026-07-24 cutover, restored 2026-09-04 after the 2026-08-30→09-04 agentpair pair was rolled back; serves Zed `deepseek_coder` FIM, Minuet suffix-FIM, and remote VSCode FIM). The `agentpair:*` tags (fim, agent, agent-gemma4-e4b, agent-gemma4-12b) stay on the PVC built but unwarmed — calling one evicts the FIM model, and re-entering that posture also needs `MAX_LOADED_MODELS` back at 2. zeta2.1 + qwen2.5-coder:fim retired from the warm list (BUG-1037: open-weights Zeta degenerates below ~3k prompt tokens vs Zed's hardcoded ~500-token self-hosted excerpts; tags kept on the PVC for rollback). gemma4:12b-it-qat orphaned (Hermes/mem0 retired, no longer warmed); OV's `gemma4:31b-cloud` VLM tag routes through this pod but occupies no local slot
- **Runs on the host, not in a pod** (IMPR-1075, 2026-09): `ollama.service` on timmy with the drop-in in `llama/host/`; the cluster reaches it through a selector-less Service + EndpointSlice (`llama/ollama-service.yaml`) and runs model jobs against it as CronJobs (`llama/ollama-jobs.yaml`). Runbook and day-to-day: `llama/host/README.md`
- **Endpoint**: `http://ollama.llama.svc:11434` (selector-less ClusterIP → host) or `192.168.1.19:11434` / `100.95.215.105:11434` (host socket, LAN / Tailscale)
- **Backend**: Vulkan on the discrete RX 9070 XT (`OLLAMA_VULKAN=1`, `OLLAMA_LLM_LIBRARY=vulkan`, `GGML_VK_VISIBLE_DEVICES=0` — RADV enumerates in PCI order, so the discrete card is device 0 and the Raphael iGPU is device 1; BUG-1094)
- **Config**: `OLLAMA_MAX_LOADED_MODELS=1` (one pinned runner, no co-load — dropped from 2 at the 2026-07-24 cutover, raised to 2 for the agentpair pair, back to 1 on 2026-09-04), `OLLAMA_NUM_PARALLEL=4` (4 × num_ctx 8192 — 3 summarize + 1 FIM, same KV as the old 2 × 16384; IDEA-1105), `OLLAMA_CONTEXT_LENGTH=131072` (TASK-1188/IMPR-1114; KV allocated on demand), `OLLAMA_KV_CACHE_TYPE=q8_0` (bumped from `q4_0` — 4.2 GB headroom at 2 slots covers it), `OLLAMA_KEEP_ALIVE=-1` (pinned). All of it lives in `llama/host/ollama.service.d/homelab.conf`; `CAP_PERFMON` is no longer needed — the host daemon reads VRAM directly
- **Deploy**: `sudo llama/host/install-host-ollama.sh` on timmy (binary pinned by `OLLAMA_VERSION` in that script) + `kubectl apply -f llama/ollama-service.yaml -f llama/ollama-jobs.yaml`

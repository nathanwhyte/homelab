...
# Benchmark Qwen3-8B on manu's GTX 1080 via port-forward
# Uses the same 5 summarization rounds as the main showdown
...
print('healthy')
" 2>/dev/null; then
    echo "Error: qwen-summarizer not healthy on port ${PORT}" >&2
    exit 1
fi

echo "Qwen3-8B on manu is healthy. Running 5 rounds..."
echo ""

# System prompts (same as main showdown, /no_think variant)
...
# Round inputs (same as main showdown)
...
CONFIG="E"  # E = Qwen3-8B on manu (CUDA)
...
echo ""
echo "Done. Results in $OUTPUT_DIR/R*_E.md"

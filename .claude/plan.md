...
### 3. Keep Vulkan in the production Ollama deployment

`llama/ollama-deployment.yaml` is now the production Vulkan deployment; the old benchmark-only `llama/ollama-deployment-vulkan.yaml` variant was removed. The production deployment should:

- Use `ollama/ollama:0.31.1`.
- Avoid `HIP_VISIBLE_DEVICES`, `rocblas-library` volumes, and ROCm hostPath mounts.
- Set `OLLAMA_VULKAN=1`, `OLLAMA_LLM_LIBRARY=vulkan`, and `GGML_VK_VISIBLE_DEVICES=1` so timmy selects the discrete RX 9070 XT via Vulkan even when ROCm runner libraries are present.
...
2. Apply the production Ollama deployment, ensure `gemma4:12b-it-qat` loads on the discrete GPU through Vulkan.
...
- **Production disruption**: The production Ollama pod is already Vulkan. If Vulkan becomes unstable, rollback now means editing `llama/ollama-deployment.yaml` rather than applying a separate ROCm variant.

### 8. Artifacts to commit

- New TOML configs in `benchmarks/ollama/configs/`.
- Updates to `llama/ollama-deployment.yaml` if the production Vulkan settings change.
...
- Gate 2: Production Ollama deployment loads `gemma4:12b-it-qat` on the RX 9070 XT through Vulkan (not iGPU/ROCm) and serves requests.
- Gate 3: Q5_K_M / Q6_K models fit in 16 GB at the chosen NUM_PARALLEL/context.

## Recommended first step now

Use `llama/ollama-deployment.yaml` for production Vulkan and keep benchmark Jobs pointed at that deployment; do not recreate the deleted benchmark-only Vulkan deployment variant.
